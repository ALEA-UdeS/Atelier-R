...
# Quelques présentations sont de mises...

<div style='text-align:center;'>
<img src="assets/img/JP.png" height="450"></img>
</div>
...
# La programmation en recherche: Exemple simple

.pull-left[
<img src="assets/img/Automatique1.png" width="100%"></img>
]

.pull-right[
<img src="assets/img/Automatique2.png" width="100%"></img>
]
...
> - *Un <b>algorithme</b> est une suite d'actions qui sont réalisées dans un ordre précis. C'est une séquence d'étapes dans la résolution d'un problème.*
...
```
PROGRAMME DEMO
  POUR CHAQUE VALEUR DE t ALLANT DE 1 À 100
    n_t = n_t * lambda
    AFFICHER n_t
    SI n_t < 1
      ARRETER
...
```
PROGRAMME DEMO
  POUR CHAQUE VALEUR DE t ALLANT DE 1 À 100 <--- Opération itérative
    n_t = n_t * lambda
    AFFICHER n_t <------------- Le programme affiche la valeur à l'écran
    SI n_t < 1 <--------------- Opération décisionnelle
      ARRETER <---------------- Le programme arrête son éxécution
    SINON
      CONTINUER <-------------- Le programme continue son éxécution
    FIN DU "SI"
  FIN DU "POUR CHAQUE"
```
...
Avec les mots clés habituels en Pseudo-code, on peut réécrire:
...
```
PROGRAMME DEMO
  TANT QUE n_t EST PLUS PETIT QUE K
    n_t = n_t * lambda
  FIN DU "TANT QUE"
```
...
```
PROGRAMME DEMO
  SI lambda EST PLUS GRAND QUE 0
    AFFICHER "La population est croissante"
  SINON lambda < 0
    AFFICHER "La population est décroissante"
  FIN DU "SI"
```
...
# Introduction aux types d'objets
...
```
# Avec le symbole "#", on peut écrire des commentaires!
# Ceci peut êttre utile pour détailler notre code.
#
# Ici, on fait la somme de deux nombres
...
Dans un fichier de code (tout comme dans n'importe quel fichier informatique!), on ne devrait jamais garder les choses qui ne servent à rien. On doit faire du ménage!
...
Programmer, c'est avant tout résoudre des problèmes. Et, si l'histoire nous a appris une chose, c'est bien qu'il vaut mieux diviser pour régner.
...
```
# Lorsqu'on ecrit un algorithme, on travaille avec des objets (ou encore des variables)
# Il vaut mieux bien nommer ces objets, sans quoi le code devient illisible et complexe!
...
On ne se cachera pas que programmer, c'est difficile au début. On doit apprendre une nouvelle langue et penser différemment. C'est important d'être fier et satisfait de nos petites victoires!
...
Un des buts de la programmation, c'est aussi de ne pas avoir à répéter les mêmes tâches encore et encore. C'est aussi vrai pour un algorithme! 
...
Bien que ce n'est pas la chose la plus importante lorsqu'on apprend à coder, il vient un temps où il est important de faire des algorithmes efficaces. Si on a besoin d'utiliser Mammouth, on ne veut pas gaspiller du temps de calcul! 
...
# Mon premier pseudo-code!

On demande à un utilisateur d'entrer son age au claver. Par la suite, le programme affiche à l'écran un message si l'utilisateur est un adulte ou pas.

Écrivez le pseudo-code pour ce programme.

---

# Mon premier pseudo-code! Solution
...
Prenez le temps de distinguer les étapes que vous réalisez lorsque vous triez les lettres. Essayez de résoudre le problème (ou une version plus simple, par exemple avec 3 lettres) "à bras". Essayez ensuite de décrire votre résolution sous forme de pseudo-code.
